...
-- but also result in conflicting type unification because let polymorphism is
-- not yet supported.
-- map_unit :: List a -> List Unit
-- map_unit = map (\x -> Unit)

-- assert map_unit \[] -> []
-- assert map_unit \[0] -> [Unit]
-- assert map_unit \[0, 0] -> [Unit, Unit]
